...
public class SideCollider : MonoBehaviour {
    ...
    public const float PrecisionEpsilon = 0.02f;
    ...
    public RaycastHit2D[] TouchingBottomObjs(LayerMask l) {
        Vector3 bottomLeft = _hitbox.bounds.min+new Vector3(0, -PrecisionEpsilon, 0);
        Vector3 localScale = transform.localScale;
        RaycastHit2D[] bottomLeftRaycast = Physics2D.RaycastAll(bottomLeft, Vector2.right, _hitbox.size.x*localScale.x, l);
        Debug.DrawLine(bottomLeft, bottomLeft+localScale.x*_hitbox.size.x*Vector3.right);
